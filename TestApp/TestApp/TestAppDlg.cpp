...
BOOL CTestAppDlg::OnInitDialog()
{
	...
	if(Stype==1)
	{
		str.Format("Software will expire in %d RUNS",DaysRemain);
		GetDlgItem(IDC_RUNS)->SetWindowTextA(str);
	}
	else
	{
		str.Format("Software will expire in %d DAYS",DaysRemain);
		GetDlgItem(IDC_RUNS)->SetWindowTextA(str);
	}
