...
{	
 ...
 wchar_t wtext[200];
 mbstowcs(wtext, m_systemfolder, strlen(m_systemfolder)+1);//Plus null
 LPWSTR ptr = wtext;

	GetSystemDirectory(ptr,MAX_PATH);
...
bool CExpire::CreateProgKey(const char*  ProgName, const char* KeyName)
{
	...
	DWORD dwDisposition;
	LONG ret;
	ret=RegCreateKeyEx(HKEY_CLASSES_ROOT,CA2CT(SubKey),0,CA2CT(""),REG_OPTION_NON_VOLATILE,
...
bool CExpire::GetRunCount(UINT& count)
{
	...
	ret=RegQueryValueEx(m_hKey,CA2CT("windll"),0,NULL,(BYTE*)data,&size);
	if(ret==ERROR_SUCCESS)
	{
...
bool CExpire::SetRunCount()
{
	...
	ret=RegSetValueEx(m_hKey,CA2CT("windll"),0,REG_SZ,(BYTE*)str,size);
	if(ret==ERROR_SUCCESS)
		return false;
	else
		return true;
...
{
	if(m_first)
		RegDeleteValue(m_hKey,CA2CT("windll"));
	return m_first;
}
...
bool CExpire::LocateFile()
{
	...
	HANDLE hFile=FindFirstFile(CA2CT(m_filename),&tmpWIN32_FIND_DATA);
